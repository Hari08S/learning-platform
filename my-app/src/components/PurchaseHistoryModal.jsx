    ...
    async function loadPurchases() {
      ...
      try {
        ...
        if (ct.includes('text/html')) {
          throw new Error('Server returned HTML for /api/me/progress — API may be misconfigured.');
        }
  ...
  return (
    <div className="modal-backdrop" style={{
      ...
      <div className="modal" style={{
        width: 920, maxWidth:'95%', maxHeight: '80vh',
        background:'#fff', borderRadius:12, padding:0, boxShadow:'0 10px 40px rgba(2,6,23,0.2)',
        display:'flex', flexDirection:'column', overflow: 'hidden'
      }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:18, borderBottom: '1px solid rgba(15,23,42,0.06)' }}>
        ...
        <div style={{
          ...
          {loading ? (
            ...
            <div style={{ color:'#6b7280', padding: 18 }}>You have no purchases yet.</div>
          ) : (
            <div style={{ display:'grid', gap:12 }}>
              {purchases.map((p) => (
                <div key={String(p.courseId)} style={{
                  display:'flex', gap:12, alignItems:'center', borderRadius:8, padding:12, background:'#f9fafb'
                }}>
                  <img src={p.img || '/logo.png'} alt={p.title} style={{ width:160, height:90, objectFit:'cover', borderRadius:6 }} />
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:700 }}>{p.title}</div>
                    <div style={{ color:'#6b7280', marginTop:4 }}>{p.author}</div>
                    <div style={{ marginTop:8, fontSize:13, color:'#374151' }}>
                      Price: ₹{p.price || '—'} • Purchased: {p.purchasedAt ? new Date(p.purchasedAt).toLocaleString() : '—'} • Status: <strong>{p.status}</strong>
                    </div>
                  </div>

                  <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                    {p.status === 'active' ? (
                      <button className="btn outline" onClick={() => doCancel(p.courseId)}>Cancel</button>
                    ) : (
                      <button className="btn primary" onClick={() => doRestore(p.courseId)}>Restore</button>
                    )}
                    <Link className="btn" to={`/courses/${p.courseId}`} onClick={onClose} style={{ textDecoration:'none', textAlign:'center' }}>Open</Link>
                  </div>
                </div>
              ))}
            </div>
