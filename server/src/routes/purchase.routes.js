...
const { isMeaningfulTitle, isSentinelString } = require('../utils/helpers');
    ...
    try {
        ...
        (user.purchasedCourses || []).forEach(pc => {
            ...
            goodPurchases.push({
                ...
                purchasedAt: pc.purchasedAt || null,
        ...
        if (Array.isArray(user.progress) && user.progress.length > 0) {
            ...
            user.progress.forEach(p => {
                const cid = p && p.courseId && p.courseId._id
                    ? String(p.courseId._id)
                    : p && p.courseId ? String(p.courseId) : null;
                if (cid) ids.add(cid);
            });

            const courseIds = Array.from(ids);
            if (courseIds.length === 0) return res.json({ purchases: [] });
    ...
    try {
        ...
        if (resolvedCourse) {
            const already = (user.purchasedCourses || []).some(pc => {
                const pcCid = pc.courseId && pc.courseId._id
                    ? String(pc.courseId._id)
                    : pc.courseId ? String(pc.courseId) : null;
        ...
        return res.status(201).json({ success: true, message: 'Purchased', purchase: created });
    } catch (err) {
        next(err);
    }
});
 ...
 */
router.delete('/purchases/:id', requireAuth, async (req, res, next) => {
    try {
        const userId = req.userId;
        ...
        (user.purchasedCourses || []).forEach(pc => {
            ...
            // Handle both populated courseId objects and raw ObjectIds
            let cid = null;
            if (pc.courseId) {
                if (typeof pc.courseId === 'object' && pc.courseId._id) {
                    cid = String(pc.courseId._id);
                } else {
                    cid = String(pc.courseId);
                }
            }
 ...
 */
router.post('/purchases/:id/restore', requireAuth, async (req, res, next) => {
    try {
        const userId = req.userId;
        const idParam = req.params.id;
        if (!userId) return res.status(401).json({ message: 'Unauthorized' });
        ...
        (user.purchasedCourses || []).forEach(pc => {
            ...
            if ((purchaseId && purchaseId === String(idParam)) || (cid && cid === String(idParam))) {
                found = true;
                pc.status = 'active';
                pc.cancelledAt = null;
            }
